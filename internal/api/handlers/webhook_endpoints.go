...
func (h *WebhookEndpointHandler) List(w http.ResponseWriter, r *http.Request) {
	items, err := h.service.List(r.Context(), callerFromRequest(r))
	if err != nil {
...
func (h *WebhookEndpointHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	ep, err := h.service.Create(r.Context(), callerFromRequest(r), webhooksapp.CreateRequest{
		URL:        body.URL,
		Secret:     body.Secret,
...
func (h *WebhookEndpointHandler) Update(w http.ResponseWriter, r *http.Request) {
	...
	ep, err := h.service.Update(r.Context(), callerFromRequest(r), epID, webhooksapp.UpdateRequest{
		URL:        body.URL,
		EventTypes: body.EventTypes,
...
func (h *WebhookEndpointHandler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.service.Delete(r.Context(), callerFromRequest(r), epID); err != nil {
		respondAppError(w, h.logger, err)
		return
	}
	noContent(w)
...
// callerFromRequest lifts the identity the webhook service needs out of the
// request context, including the scope lookup the service cannot perform for
// itself.
func callerFromRequest(r *http.Request) webhooksapp.Caller {
	caller := webhooksapp.Caller{
		Actor:    middleware.ActorFromContext(r.Context()),
		HasScope: func(scope string) bool { return middleware.HasScope(r.Context(), scope) },
	}
