...
func(h *OutboundHandler)outboundContentAllowed(ctx context.Context,actor authz.Actor,job *models.OutboundJob)(bool,error){
 if job==nil||actor.TenantID!=job.TenantID||!actor.Permission.AllowsZone(job.ZoneID){return false,nil}
 uid:=actor.EffectiveUserID();if uid==nil{return false,nil}
 u,err:=h.store.GetUser(ctx,*uid);if err!=nil{return false,err}
 if u==nil||!u.IsActive||(u.TenantID!=job.TenantID&&!(actor.IsSuperAdmin&&u.Role==models.RoleSuperAdmin)){return false,nil}
 if job.SenderUserID!=nil&&*job.SenderUserID==*uid{return true,nil}
 if job.SenderMailboxID==nil{return false,nil}
 mb,err:=h.store.ForTenant(job.TenantID).GetMailbox(ctx,*job.SenderMailboxID)
 if err!=nil{return false,err}
 if mb==nil||mb.ZoneID!=job.ZoneID{return false,nil}
...
func(h *OutboundHandler)redactOutboundJob(ctx context.Context,actor authz.Actor,job *models.OutboundJob)(*models.OutboundJob,error){
 allowed,err:=h.outboundContentAllowed(ctx,actor,job);if err!=nil{return nil,err}
 cp:=*job;cp.DeliveryToken=nil;cp.RawMIME=nil;cp.ContentRedacted=!allowed
 if !allowed{
  cp.TextBody="";cp.HTMLBody="";cp.BCC=nil;cp.HeadersJSON=nil
  cp.RcptTo=append(append([]string{},job.To...),job.CC...)
  if cp.LastError!=""{cp.LastError="Delivery details restricted; inspect status and SMTP code"}
  if cp.SMTPResponse!=""{cp.SMTPResponse="Protocol response restricted"}
 }
 return &cp,nil
}
func(h *OutboundHandler)writeOutboundView(w http.ResponseWriter,r *http.Request,job *models.OutboundJob){
 ...
 if err!=nil{errInternal(w);return};ok(w,view)
}
...
func(h *OutboundHandler)authorizeOutboundRetry(ctx context.Context,job *models.OutboundJob)error{
 ...
 if job.SenderMailboxID!=nil{
  mb,err:=h.store.ForTenant(job.TenantID).GetMailbox(ctx,*job.SenderMailboxID);if err!=nil{return err}
  return authz.CheckMailboxSender(ctx,h.store,actor,mb,job.TemplateName!=nil)
 }
 uid:=actor.EffectiveUserID()
 if uid!=nil&&job.SenderUserID!=nil&&*uid==*job.SenderUserID{return nil}
 if actor.Type==authz.PrincipalAPIKey&&job.SenderKeyID!=nil&&actor.ID==*job.SenderKeyID{return nil}
