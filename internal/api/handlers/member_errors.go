...
func(h *UserAdminHandler)writeMemberError(w http.ResponseWriter,err error){
 ...
 case errors.Is(err,store.ErrLastAdministrator),errors.Is(err,store.ErrMemberOwnsMailbox):errConflict(w,err.Error())
 default:h.logger.Error().Err(err).Msg("guarded member mutation");errInternal(w)
 }
}
