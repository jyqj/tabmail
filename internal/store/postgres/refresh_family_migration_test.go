...
func TestReleaseRefreshFamilyMigrationBackfillAndRestart(t *testing.T) {
	...
	_, err = pool.Exec(ctx, `INSERT INTO users(id,tenant_id,email,password_hash,role,is_active) VALUES($1,'00000000-0000-0000-0000-000000000001','migration@example.test','test','user',true)`, user)
	must(t, err)
	_, err = pool.Exec(ctx, `INSERT INTO refresh_tokens(id,user_id,token_hash,expires_at) VALUES($1,$3,'migration-token-one',now()+interval '1 day'),($2,$3,'migration-token-two',now()+interval '1 day')`, first, second, user)
	must(t, err)
	...
	if count != 2 {
		t.Fatalf("existing sessions not preserved as independent families: %d", count)
	}
	var version int
