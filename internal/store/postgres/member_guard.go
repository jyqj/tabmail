...
func lockMemberTenant(ctx context.Context, tx pgx.Tx, tenant uuid.UUID) error {
 ...
 if errors.Is(err, pgx.ErrNoRows) { return store.ErrMemberNotFound }
 return err
}

func currentMemberActor(ctx context.Context, tx pgx.Tx, actor authz.Actor, tenant uuid.UUID) (authz.Actor, error) {
 if actor.Type != authz.PrincipalUser || actor.TenantID != tenant { return actor, authz.ErrForbidden("interactive company administrator required") }
 u, err := scanUser(tx.QueryRow(ctx, userSelect+` WHERE id=$1`, actor.ID))
 if err != nil { return actor, err }
 if u == nil || !u.IsActive || (u.TenantID != tenant && u.Role != models.RoleSuperAdmin) { return actor, authz.ErrForbidden("administrator no longer active in this company") }
...
func guardMemberRemoval(ctx context.Context, tx pgx.Tx, old, next *models.User) error {
 ...
 if count == 0 { return store.ErrLastAdministrator }
 return nil
}
...
func (s *PgStore) UpdateUserGuarded(ctx context.Context, actor authz.Actor, tenant, target uuid.UUID, patch models.UserAdminPatch) (*models.User, error) {
 ...
 if err = lockMemberTenant(ctx, tx, tenant); err != nil { return nil, err }
 ...
 if err != nil { return nil, err }
 if old == nil { return nil, store.ErrMemberNotFound }
 if !authz.CanManageTenantMember(actor, tenant, old.Role) { return nil, authz.ErrForbidden("cannot manage this member's role") }
 next := *old
 patch.Apply(&next)
 ...
 if patch.Role != nil && next.Role == models.RoleSuperAdmin && !actor.IsSuperAdmin { return nil, authz.ErrForbidden("only super admin can assign super_admin role") }
 if err = guardMemberRemoval(ctx, tx, old, &next); err != nil { return nil, err }
 ...
 if patch.SetPermissionProfile && patch.PermissionProfileID != nil {
  var allowed bool
  if err = tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM permission_profiles WHERE id=$1 AND (tenant_id IS NULL OR tenant_id=$2))`, *patch.PermissionProfileID, tenant).Scan(&allowed); err != nil { return nil, err }
  if !allowed { return nil, authz.ErrForbidden("permission profile unavailable in this company") }
 }
 ...
 if err != nil { return nil, err }
 if tag.RowsAffected()!=1 { return nil, store.ErrMemberNotFound }
 if err = insertMemberAudit(ctx, tx, actor, old, "user.update", map[string]any{"old_role":old.Role,"new_role":next.Role,"old_active":old.IsActive,"new_active":next.IsActive}); err != nil { return nil, err }
...
func (s *PgStore) DeleteUserGuarded(ctx context.Context, actor authz.Actor, tenant, target uuid.UUID) error {
 ...
 if err != nil { return err }
 defer tx.Rollback(ctx)
 if err = lockMemberTenant(ctx, tx, tenant); err != nil { return err }
 actor, err = currentMemberActor(ctx, tx, actor, tenant)
 if err != nil { return err }
 old, err := scanUser(tx.QueryRow(ctx, userSelect+` WHERE id=$1 AND tenant_id=$2 FOR UPDATE`, target, tenant))
 if err != nil { return err }
 if old == nil { return store.ErrMemberNotFound }
 if !authz.CanManageTenantMember(actor, tenant, old.Role) { return authz.ErrForbidden("cannot manage this member's role") }
 if actor.ID==target { return authz.ErrForbidden("cannot delete yourself") }
 if err = guardMemberRemoval(ctx, tx, old, nil); err != nil { return err }
 var owns bool
 if err = tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM mailboxes WHERE tenant_id=$1 AND owner_user_id=$2)`, tenant, target).Scan(&owns); err != nil { return err }
 if owns { return store.ErrMemberOwnsMailbox }
 if _, err = tx.Exec(ctx, `DELETE FROM tenant_api_keys WHERE owner_user_id=$1 AND tenant_id=$2`, target, tenant); err != nil { return err }
 tag, err := tx.Exec(ctx, `DELETE FROM users WHERE id=$1 AND tenant_id=$2`, target, tenant)
 if err != nil { return err }
 if tag.RowsAffected()!=1 { return store.ErrMemberNotFound }
 if err = insertMemberAudit(ctx, tx, actor, old, "user.delete", map[string]any{"role":old.Role}); err != nil { return err }
