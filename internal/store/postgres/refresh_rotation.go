...
func (s *PgStore) RotateRefreshToken(ctx context.Context, oldHash string, next *models.RefreshToken) (bool, bool, error) {
 if next==nil || next.TokenHash=="" || next.TokenHash==oldHash { return false, false, fmt.Errorf("invalid refresh replacement") }
 tx, err := s.pool.Begin(ctx)
 if err != nil { return false, false, err }
 defer tx.Rollback(ctx)
 family, err := lockRefreshFamily(ctx,tx,oldHash)
 if errors.Is(err,pgx.ErrNoRows) { return false,false,nil }
 if err!=nil { return false,false,err }
 ...
 if err!=nil { return false,false,err }
 if revoked!=nil {
  if _,err=tx.Exec(ctx,`UPDATE refresh_tokens SET revoked_at=clock_timestamp() WHERE family_id=$1 AND revoked_at IS NULL`,family);err!=nil{return false,false,err}
  if err=tx.Commit(ctx);err!=nil{return false,false,err}
  return false,true,nil
 }
 if expired || !active { return false,false,nil }
 ...
 if err!=nil{return false,false,err}
 if tag.RowsAffected()!=1{return false,false,nil}
 if next.ID==uuid.Nil{next.ID=uuid.New()}
 next.UserID=userID
 ...
 if err!=nil{return false,false,err}
 if tag.RowsAffected()!=1{return false,false,fmt.Errorf("replacement refresh token already expired")}
 if err=tx.Commit(ctx);err!=nil{return false,false,err}
...
func (s *PgStore) RevokeRefreshTokenByHash(ctx context.Context, hash string) error {
 ...
 if err!=nil{return err}
 if _,err=tx.Exec(ctx,`UPDATE refresh_tokens SET revoked_at=clock_timestamp() WHERE family_id=$1 AND revoked_at IS NULL`,family);err!=nil{return err}
