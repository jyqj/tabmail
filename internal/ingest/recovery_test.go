...
func TestIngressHeldBytesSurviveRetentionAndObjectGC(t *testing.T) {
	f := newRecoveryFixture(t)
	ctx := context.Background()
	...
	called := false
	released, err := f.st.ReleaseRawObjectIfUnreferenced(ctx, j.RawObjectKey, func(context.Context) error { called = true; return nil })
	check(t, err)
	if released || called {
		t.Fatal("held original deleted by raw-object GC")
	}
}
...
func TestIngressExpiredWorkerCannotCommitOrFinalize(t *testing.T) {
	...
	check(t, f.st.FinishIngress(ctx, fresh, 2, time.Now()))
...
func TestIngressConcurrentClaimsAndQuota(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		wg.Add(1)
		go func() { defer wg.Done(); f.svc.ProcessBatch(ctx); errs <- nil }()
	}
	wg.Wait()
	close(errs)
	for err := range errs {
...
func TestIngressMissingRawRetainedForReview(t *testing.T) {
	...
	// Operator recovery is deliberately P1. P0 retains the original receipt
	// and diagnostic target states rather than permitting an unaudited replay.
}
...
func TestIngressAuditFailureRollsBackAndCorruptionIsHeld(t *testing.T) {
 ...
 CREATE TRIGGER audit_fault BEFORE INSERT ON audit_log FOR EACH ROW EXECUTE FUNCTION reject_inbound_audit()`)
	...
	targets, err := f.st.ListIngressTargets(ctx, j.ID)
	check(t, err)
	if targets[0].LastError != "original object size/checksum mismatch; receipt held for recovery" {
		t.Fatalf("%+v", targets)
	}
}
...
func TestIngressLegacyQueuesCannotBypassLedger(t *testing.T) {
	...
	check(t, f.st.FinishIngress(ctx, c, 2, time.Now()))
