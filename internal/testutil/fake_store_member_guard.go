...
func (s *FakeStore) UpdateUserGuarded(_ context.Context, actor authz.Actor, tenant, target uuid.UUID, patch models.UserAdminPatch) (*models.User, error) {
	...
	defer s.mu.Unlock()
	var err error
	actor, err = s.memberActorLocked(actor, tenant)
	if err != nil {
		return nil, err
	}
	old := s.users[target]
	if old == nil || old.TenantID != tenant {
		return nil, store.ErrMemberNotFound
	}
	if !authz.CanManageTenantMember(actor, tenant, old.Role) {
		return nil, authz.ErrForbidden("cannot manage this member")
	}
	next := *old
	patch.Apply(&next)
	...
	if patch.Role != nil && next.Role == models.RoleSuperAdmin && !actor.IsSuperAdmin {
		return nil, authz.ErrForbidden("cannot assign super_admin")
	}
	if err = s.guardRemovalLocked(old, &next); err != nil {
		return nil, err
	}
...
func (s *FakeStore) DeleteUserGuarded(_ context.Context, actor authz.Actor, tenant, target uuid.UUID) error {
	...
	if actor.ID == target || !authz.CanManageTenantMember(actor, tenant, old.Role) {
		return authz.ErrForbidden("cannot delete this member")
	}
	if err = s.guardRemovalLocked(old, nil); err != nil {
		return err
	}
	for _, m := range s.mailboxes {
		if m.OwnerUserID != nil && *m.OwnerUserID == target {
			return store.ErrMemberOwnsMailbox
		}
	}
	for id, k := range s.apiKeys {
		if k.OwnerUserID != nil && *k.OwnerUserID == target {
			delete(s.apiKeys, id)
		}
	}
	...
	for key, g := range s.mailboxGrants {
		if g.UserID == target {
			delete(s.mailboxGrants, key)
		}
	}
