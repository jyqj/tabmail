...
import { listZoneGrants, createZoneGrant, deleteZoneGrant } from "@/lib/api";
import type { ZoneGrant, ZoneGrantRole } from "@/lib/types";
...
const roleColors: Record<ZoneGrantRole, string> = {
...
export default function ZoneGrantsPage() {
  const { t } = useI18n();
  const { level } = useAuth();
  ...
  const [role, setRole] = useState<ZoneGrantRole>("viewer");

  const fetchGrants = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listZoneGrants(zoneId);
      setGrants(res.data ?? []);
      setError(false);
    } catch {
      setError(true);
      toast.error("加载域名授权失败");
    } finally {
      setLoading(false);
    }
  }, [zoneId]);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setRole("viewer");
      setDialogOpen(false);
      toast.success("域名授权已创建");
      fetchGrants();
    } catch (err: unknown) {
      const apiErr = err as { error?: { message?: string } };
      toast.error(apiErr?.error?.message || "创建域名授权失败");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleDelete = async (grantId: string) => {
    if (!safeConfirm("确定要删除此域名授权吗？")) return;
    try {
      await deleteZoneGrant(zoneId, grantId);
      toast.success("域名授权已删除");
      fetchGrants();
    } catch {
      toast.error("删除域名授权失败");
    }
  };
          ...
          isAdmin ? (
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  <DialogTitle>添加域名授权</DialogTitle>
                  <DialogDescription>
                    授予用户或 API Key 对此域名的访问权限
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    <Label>角色</Label>
                    <Select value={role} onValueChange={(v) => v && setRole(v as ZoneGrantRole)}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
